...
import { LikeLevel } from "./like-level";
...
import { getBrightness } from "./color-util";
...
export class Song {
  ...
  private _reasonPlayedText: string | null = null;
  ...
  get communityRankStandingText(): string {
    switch (this.communityRankStanding) {
      case CommunityRankStanding.Best:
        return "Best";
      case CommunityRankStanding.Good:
        return "Good";
      case CommunityRankStanding.Great:
        return "Great";
      case CommunityRankStanding.Poor:
        return "Poor";
      case CommunityRankStanding.VeryPoor:
        return "Very Poor";
  ...
  private updateAlbumSwatchLighterDarker() {
    ...
    const bgBrightness = getBrightness(bgOrDefault);
    const fgBrightness = getBrightness(fgOrDefault);
  ...
  static empty(): Song {
    return new Song({
      ...
      totalPlays: 0,
      uri: "",
      reasonsPlayed: Song.createEmptySongPickReasons("songs/0"),
      albumColors: {
        background: "",
        foreground: "",
        muted: "",
  ...
  private createReasonPlayedText(): string {
    ...
    const randomReason =
      "Chavah plays random songs from time to time to see what kind of music you like";
    if (this.reasonsPlayed) {
      ...
      if (this.reasonsPlayed.soleReason !== null) {
        switch (this.reasonsPlayed.soleReason) {
          case SongPick.SomeoneRequestedSong:
            return "it was requested by a listener";
          case SongPick.SongFromAlbumRequested:
            return `you asked to hear another song from the ${this.album} album`;
          case SongPick.SongFromArtistRequested:
            return `you asked to hear another song from ${this.artist}`;
          case SongPick.SongWithTagRequested:
            return `you asked to hear another song with this tag`;
          case SongPick.VeryPoorRank:
            return "...well, even the lowest-ranked songs will get played sometimes :-)";
          case SongPick.YouRequestedSong:
            return "you asked Chavah to play it";
  ...
  static createEmptySongPickReasons(songId: string): SongPickReasons {
    return {
